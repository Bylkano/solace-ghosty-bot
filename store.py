...
def _init() -> None:
    ...
    with _connect() as con:
        with con.cursor() as cur:
            ...
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS drops_disabled_channels (
                    guild_id   BIGINT NOT NULL,
...
# ── All-channels mode + per-channel blacklist ─────────────────

def get_all_channels_mode(guild_id: int) -> bool:
    """True if drops should fire in every channel (except disabled ones)."""
    return bool(_get_value(guild_id, "drops_all_channels"))


def set_all_channels_mode(guild_id: int, enabled: bool) -> None:
    """Enable or disable all-channels drop mode for a guild."""
    _set_value(guild_id, "drops_all_channels", 1 if enabled else 0)


def get_drops_paused(guild_id: int) -> bool:
    """True if all drops are paused server-wide (overrides mode/channel)."""
    return bool(_get_value(guild_id, "drops_paused"))


def set_drops_paused(guild_id: int, paused: bool) -> None:
    """Pause or resume all drops for a guild."""
    _set_value(guild_id, "drops_paused", 1 if paused else 0)


def add_disabled_channel(guild_id: int, channel_id: int) -> None:
    """Block drops in a specific channel (used in all-channels mode)."""
    with _connect() as con:
        with con.cursor() as cur:
            cur.execute(
                "INSERT INTO drops_disabled_channels (guild_id, channel_id) VALUES (%s, %s) "
...
def remove_disabled_channel(guild_id: int, channel_id: int) -> None:
    """Re-enable drops in a previously blocked channel."""
    with _connect() as con:
        with con.cursor() as cur:
            cur.execute(
                "DELETE FROM drops_disabled_channels WHERE guild_id = %s AND channel_id = %s",
                (guild_id, channel_id),
            )
        con.commit()


def get_disabled_channels(guild_id: int) -> list[int]:
    """Return channel IDs where drops are disabled for a guild."""
    with _connect() as con:
        with con.cursor() as cur:
            cur.execute(
                "SELECT channel_id FROM drops_disabled_channels WHERE guild_id = %s",
                (guild_id,),
            )
