...
--------
  ...
  /tree                    – Show the full Solace family tree image
...
# Optional: cheating detection hook from crush.py.
# Degrades gracefully if the crush cog isn't installed.
try:
    from cogs.crush import fire_cheat_event as _fire_cheat_event
    _CRUSH_COG_LOADED = True
except ImportError:
    _CRUSH_COG_LOADED = False
...
def _find_user_subtree(roots: list[FamilyNode], user_id: int) -> FamilyNode | None:
    """Walk the tree and return the subtree rooted at the node containing user_id."""
    def _walk(node: FamilyNode) -> FamilyNode | None:
...
class FamilyTree(commands.Cog):
    ...
    async def marry(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if proposer_marriage:
            # Fire cheating event if crush cog is available
            if _CRUSH_COG_LOADED:
                spouse_id = (
                    proposer_marriage["user2_id"]
                    if proposer_marriage["user1_id"] == proposer.id
                    else proposer_marriage["user1_id"]
                )
                await _fire_cheat_event(
                    self.bot, guild_id, interaction.channel,
                    proposer, member.id, spouse_id, "propose",
                )
    ...
    async def family(self, interaction: discord.Interaction,
        ...
        if not roots:
            return await interaction.followup.send(
                embed=discord.Embed(
                    description="No family relationships exist in this server yet.",
                    color=0x445566,
                )
            )

        subtree = _find_user_subtree(roots, target.id)
        if subtree is None:
            return await interaction.followup.send(
                embed=discord.Embed(
        ...
        tree_roots = [subtree]
        all_ids = _collect_ids_from_roots(tree_roots)
        ...
        _fill_display_names(tree_roots, name_map)
        ...
        await status_msg.edit(
            ...
            attachments=[discord.File(img_buf, filename="family.png")],
        )

    # ── /tree ─────────────────────────────────────────────────────────────

    @app_commands.command(name="tree", description="🌲 Show the full Solace family tree.")
    @app_commands.guild_only()
    async def tree(self, interaction: discord.Interaction):
            ...
            return await interaction.followup.send(
                embed=discord.Embed(
                    ...
                    description=(
                        ...
                        "Use `/marry`, `/adopt` to start building the family tree."
                    ),
                    color=0x334455,
                )
            )
        ...
        stats_embed.set_footer(text="💞 Relationships update automatically.")
        await status_msg.edit(
            embed=stats_embed,
            attachments=[discord.File(img_buf, filename="solace_family_tree.png")],
        )
