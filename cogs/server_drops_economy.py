...
# ── Event shop ─────────────────────────────────────────────────────
BOOST_COST            = 2000   # cost of a personal point boost
BOOST_DURATION_MIN    = 30     # minutes a boost lasts
SHIELD_COST           = 1500   # cost of an anti-heist shield
SHIELD_DURATION_HOURS = 2      # hours a shield lasts
HEIST_SUCCESS_CHANCE  = 0.5    # chance a heist succeeds
HEIST_STEAL_PCT       = 0.15   # fraction of target balance stolen on success
HEIST_FAIL_PENALTY    = 300    # points lost on a failed heist
HEIST_COOLDOWN_HOURS  = 2      # hours between heists
GAMBLE_MIN            = 100    # minimum gamble bet
GAMBLE_COOLDOWN_SEC   = 30     # seconds between gambles
        ...
        with con.cursor() as cur:
            ...
            cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS economy_effects (
                    user_id           BIGINT PRIMARY KEY,
                    boost_until       DOUBLE PRECISION NOT NULL DEFAULT 0,
                    shield_until      DOUBLE PRECISION NOT NULL DEFAULT 0,
                    heist_cd_until    DOUBLE PRECISION NOT NULL DEFAULT 0,
                    last_heist_target BIGINT NOT NULL DEFAULT 0
                )
                """
            )
        con.commit()
...
# ──────────────── shop effects (boosts / shields / heist) ──────────

def _get_effects(user_id: int) -> dict:
    with _pg_connect() as con:
        with con.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT boost_until, shield_until, heist_cd_until, last_heist_target "
                "FROM economy_effects WHERE user_id = %s",
                (user_id,),
            )
            row = cur.fetchone()
    if not row:
        return {"boost_until": 0.0, "shield_until": 0.0,
                "heist_cd_until": 0.0, "last_heist_target": 0}
    return dict(row)


_EFFECT_FIELDS = ("boost_until", "shield_until", "heist_cd_until", "last_heist_target")


def _set_effect(user_id: int, field: str, value) -> None:
    if field not in _EFFECT_FIELDS:
        raise ValueError(f"unknown effect field: {field}")
    with _pg_connect() as con:
        with con.cursor() as cur:
            cur.execute(
                f"INSERT INTO economy_effects (user_id, {field}) VALUES (%s, %s) "
...
def _has_boost(user_id: int) -> bool:
    return _get_effects(user_id)["boost_until"] > time.time()
...
class ServerDropsEconomy(commands.Cog, name="ServerDropsEconomy"):
    def __init__(self, bot: commands.Bot):
        ...
        self._gamble_cd: dict[int, float] = {}  # user_id → last gamble epoch
    ...
    def _award_win(self, user_id: int, base: int) -> int:
        """Award drop winnings, doubled if the user has an active Point Boost."""
        amount = base * 2 if _has_boost(user_id) else base
    ...
    async def _potato_timer(self, channel: discord.TextChannel, state: dict):
        ...
        if random.random() < 0.5:
            ...
            # GOLDEN LOOT SACK
            new_total = add_points(holder_id, 500)
            await channel.send(
    ...
    @commands.hybrid_command(name="shop", description="View the event shop and item prices.")
    async def shop_command(self, ctx: commands.Context):
        e = _base_embed(C_POINTS)
        e.title = "🛒  Event Shop"
        e.description = (
            f"{SEP}\n"
            f"⚡ **Point Boost** — `{BOOST_COST:,} pts`\n"
            f"Doubles your drop winnings for **{BOOST_DURATION_MIN} min**.\n"
            f"› `/buy boost`\n\n"
            f"🛡️ **Shield** — `{SHIELD_COST:,} pts`\n"
            f"Blocks heists against you for **{SHIELD_DURATION_HOURS}h**.\n"
            f"› `/buy shield`\n\n"
            f"🗡️ **Heist** — *free to attempt*\n"
            f"**{int(HEIST_SUCCESS_CHANCE * 100)}%** chance to steal "
            f"**{int(HEIST_STEAL_PCT * 100)}%** of someone's points. "
            f"Fail = lose `{HEIST_FAIL_PENALTY:,} pts`. Cooldown **{HEIST_COOLDOWN_HOURS}h**.\n"
            f"› `/heist @user`\n\n"
            f"🎲 **Gamble** — *you set the bet*\n"
            f"50/50 to double your bet or lose it. Min `{GAMBLE_MIN:,} pts`.\n"
            f"› `/gamble <amount>`\n"
            f"{SEP}"
        )
        e.set_footer(text="SOLACE ECONOMY  •  Spend wisely")
        await ctx.send(embed=e)

    @app_commands.command(name="buy", description="Buy an item from the event shop.")
    @app_commands.guild_only()
    @app_commands.describe(item="The item to buy.")
    @app_commands.choices(item=[
        app_commands.Choice(name="⚡ Point Boost — 2x drops for 30 min", value="boost"),
        app_commands.Choice(name="🛡️ Shield — block heists for 2h", value="shield"),
    ])
    async def buy(self, interaction: discord.Interaction, item: app_commands.Choice[str]):
        uid = interaction.user.id
        now = time.time()
        bal = get_points(uid)
        eff = _get_effects(uid)

        if item.value == "boost":
            if eff["boost_until"] > now:
                mins = int((eff["boost_until"] - now) // 60) + 1
                await interaction.response.send_message(
                    f"⚡ You already have a boost active (~{mins} min left). "
                    "Wait for it to end before buying another.",
                    ephemeral=True,
                )
                return
            if bal < BOOST_COST:
                await interaction.response.send_message(
                    f"❌ You need `{BOOST_COST:,} pts` but only have `{bal:,} pts`.",
                    ephemeral=True,
                )
                return
            deduct_points(uid, BOOST_COST)
            _set_effect(uid, "boost_until", now + BOOST_DURATION_MIN * 60)
            await interaction.response.send_message(
                f"⚡ **Point Boost activated!** Your drop winnings are **doubled** "
                f"for the next **{BOOST_DURATION_MIN} minutes**. Go grind! 🔥"
            )
            return

        if item.value == "shield":
            if eff["shield_until"] > now:
                mins = int((eff["shield_until"] - now) // 60) + 1
                await interaction.response.send_message(
                    f"🛡️ Your shield is still active (~{mins} min left).",
                    ephemeral=True,
                )
                return
            if bal < SHIELD_COST:
                await interaction.response.send_message(
                    f"❌ You need `{SHIELD_COST:,} pts` but only have `{bal:,} pts`.",
                    ephemeral=True,
                )
                return
            deduct_points(uid, SHIELD_COST)
            _set_effect(uid, "shield_until", now + SHIELD_DURATION_HOURS * 3600)
            await interaction.response.send_message(
                f"🛡️ **Shield activated!** Heists against you will fail for the "
                f"next **{SHIELD_DURATION_HOURS} hours**."
            )
            return

    @app_commands.command(name="gamble", description="Bet points on a 50/50 double-or-nothing.")
    @app_commands.guild_only()
    @app_commands.describe(amount="How many points to bet.")
    async def gamble(self, interaction: discord.Interaction, amount: int):
        uid = interaction.user.id
        now = time.time()
        last = self._gamble_cd.get(uid, 0.0)
        if now - last < GAMBLE_COOLDOWN_SEC:
            wait = int(GAMBLE_COOLDOWN_SEC - (now - last)) + 1
            await interaction.response.send_message(
                f"🎲 Slow down! You can gamble again in **{wait}s**.", ephemeral=True
            )
            return
        if amount < GAMBLE_MIN:
            await interaction.response.send_message(
                f"❌ Minimum bet is `{GAMBLE_MIN:,} pts`.", ephemeral=True
            )
            return
        bal = get_points(uid)
        if bal < amount:
            await interaction.response.send_message(
                f"❌ You only have `{bal:,} pts`.", ephemeral=True
            )
            return
        self._gamble_cd[uid] = now
        if random.random() < 0.5:
            new_total = add_points(uid, amount)
            e = _base_embed(C_WIN)
            e.description = (
                f"🎲 **You WON!** 🎉\n"
                f"＋`{amount:,} pts`  ›  Balance: `{new_total:,} pts`"
            )
        else:
            new_total = deduct_points(uid, amount)
            e = _base_embed(0xE74C3C)
            e.description = (
                f"🎲 **You lost...** 💀\n"
                f"−`{amount:,} pts`  ›  Balance: `{new_total:,} pts`"
            )
        e.set_footer(text="SOLACE ECONOMY  •  Gamble")
        await interaction.response.send_message(embed=e)

    @app_commands.command(name="heist", description="Attempt to steal points from another member.")
    @app_commands.guild_only()
    ...
    async def heist(self, interaction: discord.Interaction, target: discord.Member):
        ...
        if target.id == uid:
            await interaction.response.send_message("❌ You can't heist yourself.", ephemeral=True)
            return
        if target.bot:
            await interaction.response.send_message("❌ You can't heist a bot.", ephemeral=True)
            return

        eff = _get_effects(uid)
        if eff["heist_cd_until"] > now:
            mins = int((eff["heist_cd_until"] - now) // 60) + 1
            await interaction.response.send_message(
                f"🗡️ Your crew is laying low. Next heist in **~{mins} min**.", ephemeral=True
            )
            return
        if eff["last_heist_target"] == target.id:
            await interaction.response.send_message(
                "❌ You just hit that person — pick a different target.", ephemeral=True
            )
            return

        # Lock in cooldown + last target regardless of outcome
        _set_effect(uid, "heist_cd_until", now + HEIST_COOLDOWN_HOURS * 3600)
        _set_effect(uid, "last_heist_target", target.id)

        if _get_effects(target.id)["shield_until"] > now:
            await interaction.response.send_message(
                f"🛡️ **{target.display_name} is shielded!** Your heist bounced right off. "
                "Cooldown started anyway."
            )
            return

        target_bal = get_points(target.id)
        if random.random() < HEIST_SUCCESS_CHANCE:
            steal = int(target_bal * HEIST_STEAL_PCT)
            if steal <= 0:
                await interaction.response.send_message(
                    f"🗡️ You cracked **{target.display_name}**'s vault... but it was empty. 😬"
                )
                return
            deduct_points(target.id, steal)
            new_total = add_points(uid, steal)
            e = _base_embed(C_WIN)
            e.description = (
                f"🗡️ **HEIST SUCCESS!**\n"
                f"You stole `{steal:,} pts` from **{target.display_name}**!\n"
                f"Your balance: `{new_total:,} pts`"
            )
        else:
            new_total = deduct_points(uid, HEIST_FAIL_PENALTY)
            e = _base_embed(0xE74C3C)
            e.description = (
                f"🚨 **HEIST FAILED!**\n"
                f"You got caught robbing **{target.display_name}** and paid a "
                f"`{HEIST_FAIL_PENALTY:,} pt` fine.\n"
                f"Your balance: `{new_total:,} pts`"
            )
        e.set_footer(text="SOLACE ECONOMY  •  Heist")
        await interaction.response.send_message(embed=e)
