...
def leave_family(guild_id: int, user_id: int) -> int:
    """
    Child-initiated: remove the user from every active parent-child record
    where they are the child. Returns the number of parent links removed.
    Marriage and own children are untouched.
    """
    with _connect() as con:
        with con.cursor() as cur:
            cur.execute(
...
def get_longest_couples(guild_id: int, limit: int = 5) -> list[dict]:
    """Active couples ordered by duration (longest first)."""
    with _connect() as con:
        with con.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT user1_id, user2_id, married_at,
                       EXTRACT(DAY FROM NOW() - married_at)::INT AS days
                FROM ft_marriages
                WHERE guild_id = %s AND divorced_at IS NULL
...
def get_top_parents(guild_id: int, limit: int = 5) -> list[dict]:
    """Users with the most active children."""
    with _connect() as con:
        with con.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT parent_id, COUNT(*) AS child_count
                FROM ft_parent_child
                WHERE guild_id = %s AND disowned_at IS NULL
                GROUP BY parent_id
...
def get_most_married(guild_id: int, limit: int = 5) -> list[dict]:
    """Users with the most marriages ever (including divorced)."""
    with _connect() as con:
        with con.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT user_id, COUNT(*) AS marriage_count FROM (
                    SELECT user1_id AS user_id FROM ft_marriages WHERE guild_id = %s
                    UNION ALL
                    SELECT user2_id AS user_id FROM ft_marriages WHERE guild_id = %s
                ) t
                GROUP BY user_id
